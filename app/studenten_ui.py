...
def run_student_ui() -> None:
    ...
    with Session(ENGINE) as ses:
        ...
        if not students:
            st.warning(f"Keine Schüler für Klasse **{class_sel}** in der Datenbank.")
            if st.button("📂 Schüler importieren (Setup öffnen)", key="_stu_goto_setup"):
                st.session_state["_page_switch"] = "⚙️ Setup"
                st.rerun()
            return
        if not topics_raw:
